...
for SYS in ${SYSTEMS[@]} ; do
    ...
    for V in {00..00..1} ; do
        ...
        # go to sub-directory
        mkdir -p v$V/$OUT_ROOT/mmgbsa
        cd v$V/$OUT_ROOT/mmgbsa
        ...
        IN="$IN / \n"
        # here molsurf=1 is molsurf algorithm, =2 is LCPO (default)
        IN="$IN &gb \n"
        IN="$IN    molsurf=1, \n"
...
#       IN="$IN   dec_verbose=1, \n"
        ...
        $DO_PARALLEL \
        $AMBERHOME/bin/MMPBSA.py.MPI -O -i mmgbsa.in -o MMGBSA_RESULTS_SURF1.dat -eo MMGBSA_ENERGY_OUT_SURF1.dat -cp ${PDB}_${SYS}_vac.prmtop -y ../../??_prod_dry.nc > progress.log 2>&1
