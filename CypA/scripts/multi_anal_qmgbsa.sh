...
for SYS in ${SYSTEMS[@]} ; do
    ...
    for V in {00..04..1} ; do
        ...
        # go to sub-directory
        mkdir -p v$V/$OUT_ROOT/qmgbsa
        cd v$V/$OUT_ROOT/qmgbsa
        ...
        IN="$IN / \n"
        # here run QM/MM GBSA at PM3 semi-empirical level
        IN="$IN &gb \n"
        # set QM on (ifqnt) for 121 only using PM3
        IN="$IN    ifqnt=1, qm_residues='121', qm_theory='PM3'\n"
        IN="$IN    igb=5, saltcon=0.0 \n"

        # QM and decmop are incompatible
#        IN="$IN / \n"
...
#        IN="$IN   dec_verbose=1, \n"
        ...
        $DO_PARALLEL \
        $AMBERHOME/bin/MMPBSA.py.MPI -O -i qmgbsa.in -o QMGBSA_RESULTS.dat -eo QMGBSA_ENERGY_OUT.dat -do QMGBSA_DECOMP_RESULTS.dat -deo QMGBSA_DECOMP_ENERGY_OUT.dat -cp ${PDB}_${SYS}_vac.prmtop -y ../../??_prod_dry.nc > progress.log 2>&1
