...
# co to conformers directory
cd v02/GenConformers
...
cp -v ../../../ipq_qm_single_conf_setup.sh .

# make mdgx file and run with sbatch
cat << EOF > conf_${CONF}_mdgx_grid_gen.slurm
#!/bin/bash
#SBATCH --job-name=conf-${CONF}-W4F-ipq-mdgx-grid-gen
#SBATCH --cluster=smp
...
bash ipq_qm_single_conf_setup.sh ${CONF} &&
