...
def plot_ramachandran(XY, Z, Zdiff=None, ax=None, pmax=5, cmap="afmhot", 
    ...
    if Zdiff is not None:
        ...
        # print min and max ∆∆G
        print(f"{np.mean(Z[np.logical_not(np.isnan(Z))]):.3f}±{np.std(Z[np.logical_not(np.isnan(Z))]):.3f}\t{np.min(Z[np.logical_not(np.isnan(Z))]):.3f}\t{np.max(Z[np.logical_not(np.isnan(Z))]):.3f}")
    else:
        pmin = 0
    if ax is None:
    ...
    ax.set_xlabel(r"$\Phi$", labelpad=4, fontweight="bold", fontsize=24)
    ax.set_xticks(ticks)
    ax.set_xticklabels(tick_labels, fontsize=16)
    ax.set_ylabel(r"$\Psi$", rotation=1, va='center', ha='left', 
                  labelpad=18, fontweight="bold", fontsize=24)
    ax.set_yticks(ticks)
    ...
    if state_labels is True:
        for state in states:
            label = ax.text(state[1], state[2], state[0], ha="center", va="center", weight="bold", size=12)
...
XY, Z = process_wham(f"PMFs/W4F_pmf.dat")
plot, lines = plot_ramachandran(XY, Z, title="AMBER ff15ipq", state_labels=False, cbar=True)
plt.savefig("figures/w4f_toc.png", dpi=900, transparent=True)
#plt.show()
...
def multi_pmf_plot():
    ...
    for ax, res in zip(grid[:len(systems)], systems):
        ...
        if res == "TRP" or res == "TYR" or res == "PHE":
            ...
            ax.scatter(pdb_data[:, 0], pdb_data[:, 1], color="green", s=20)
        
    # add phi/psi diff data to image grid : ∆∆G
    for ax, res in zip(grid[len(systems):], systems):
        # hide the same res diff plots
        # if res == systems[0]:
        #     ax.axis("off")
        # else:
        ...
        #print(f"# Residue Class\tAverage ∆∆G (kcal/mol)\tMin ∆∆G(kcal/mol)\tMax ∆∆G(kcal/mol)")
    ...
    fig.tight_layout()
    plt.show()
    #fig.savefig("figures/trp_diff_bold.pdf", dpi=600, transparent=True)
