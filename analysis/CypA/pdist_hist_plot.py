...
def multi_cypa_dihedral_plot(type="ramachandran"):
    ...
    fig, axes = plt.subplots(nrows=1, ncols=len(systems), figsize=(11, 3), sharey=True, # 15, 3.5 was OG size
    ...
    for res, ax in zip(systems, axes):
        ...
        if res == "wt":
            dh = Get_Dihedrals(f"CYPA_XTALS/3k0n_cypa_leap.pdb", "TRP")
        else:
            dh = Get_Dihedrals(f"CYPA_XTALS/{res.upper()}CypA.pdb", res.upper())
    ...
    cbar = fig.colorbar(plot, pad=0.1, aspect=15)
    cbar.set_label("-ln P(x)", weight="bold", labelpad=10)
    axes[0].set_ylabel(y, weight="bold", labelpad=-1, rotation=0, ha="center", va="center")
    fig.tight_layout()
    plt.show()
    #fig.savefig(f"figures/cypa_trp121_{type}_5us.svg", dpi=900, bbox_inches='tight', transparent=True)
...
def confs_phe113_rms_plot():
    ...
    for res, ax in zip(systems, axes):
        plot_options = {"xlabel" : "F113 RMSD (3K0N)", 
                        #"ylabel" : "121 RMSD to $^{19}F CypA$",
                        "title" : res.upper() + " CypA",
                        "xlim" : (0, 6),
                        #"xticks" : np.arange(2,8,1),
        ...
        major_ref_paths = [f"ipq/{res}/v0{i}/1us_noion/rmsd_phe113_3k0n.dat" for i in range(0,5)]
        minor_ref_paths = [f"ipq/{res}/v0{i}/1us_noion/rmsd_phe113_3k0o.dat" for i in range(0,5)]
                
        # phi in second col, psi in third col
        X, Y, Z = pre_processing(major_ref_paths, minor_ref_paths, x_loc=1, y_loc=1)
        plot = pdist_plot(X, Y, Z, pmax=5, ax=ax, cmap="afmhot_r", **plot_options)
        # draw diagonal line
        ax.plot([0, 1], [0, 1], transform=ax.transAxes, linestyle="--", color="k")

    cbar = fig.colorbar(plot, pad=0.1)
    cbar.set_label("-ln (P(x))", weight="bold")
    axes[0].set_ylabel("F113 RMSD (3K0O)", weight="bold")
    fig.tight_layout()
    plt.show()
    #fig.savefig(f"figures/cypa_rms_f113_3k0n_vs_3k0o_5us.png", dpi=300, transparent=True)

confs_phe113_rms_plot()